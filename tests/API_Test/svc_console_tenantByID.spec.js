...
test('API_svcConsole_TenantByID_Test: Get Tenant by ID API', async ({ page }) => {
  let accessToken = '';
  ...
  const tenantId = 7; // Replace with the actual tenant ID
  const apiUrl = `${apiEndpoints.tenantById}/${tenantId}`;
  ...
  expect(responseBody).toHaveProperty('id'); // Example assertion for a property
});