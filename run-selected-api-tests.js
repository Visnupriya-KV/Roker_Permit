...
async function sendReportEmail(status) {
    ...
    const dynamicFilename = `QA_permit_${formattedDate}/${formattedTime}.html`;

    const mailOptions = {
        from: emailConfig.from,
        to: emailConfig.to,
        subject: `QA ROKER Permit TEST REPORT - ${status.toUpperCase()} - (${currentDateTime})`,
        text: `Hi Team,

QA Roker Permit test execution has completed with the following details:
...
QA Automation`,
        html: `<p>Hi Team,</p>
<p>QA Roker Permit test execution has completed with the following details:</p>
<ul>
